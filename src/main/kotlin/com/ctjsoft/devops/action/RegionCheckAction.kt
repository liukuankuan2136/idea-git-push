...
class RegionCheckAction : AnAction() {
    override fun actionPerformed(event: AnActionEvent) {
        ...
        ProgressManager.getInstance().run(object : Task.Backgroundable(project, "加载区域检查范围", false) {
            ...
            override fun onSuccess() { RegionCheckDialog(project, api, projects, products).show() }
            override fun onThrowable(error: Throwable) = ActionSupport.notify(project, "区域检查加载失败", error.message ?: "加载失败。", NotificationType.ERROR)
        })
    }
}
